import { AsyncRetriable, mapError, ignoreZero, areBytes32Equal, tapError, formatEid, makeBytes32, isZero, denormalizePeer, normalizePeer, createDefaultApplicative } from '@layerzerolabs/devtools';
import { OmniSDK, BigNumberishNumberSchema, BigNumberishBigIntSchema, parseGenericError, formatOmniContract, RevertError } from '@layerzerolabs/devtools-evm';
import { z } from 'zod';
...
import { MaxUint256, One, NegativeOne, Zero } from '@ethersproject/constants';
import { ComputeSetting } from '@layerzerolabs/lz-v2-utilities';
import { UnresolvableCommandError, isBlockMatchingTimestamp, dedup } from '@layerzerolabs/ua-devtools';
...
// src/ownable/sdk.ts
var Ownable = class extends OmniSDK {
  async getOwner() {
...
var SymbolSchema = z.string();
var createERC20Factory = (contractFactory) => pMemoize(async (point) => new ERC20(await contractFactory(point)));
var LzApp = class extends OmniSDK {
...
var createLzAppFactory = (contractFactory) => pMemoize(async (point) => new LzApp(await contractFactory(point)));
var OApp = class extends Ownable {
  async getEndpointSDK() {
    ...
    return new EndpointV2(this.contract.contract.provider, { address, eid: this.point.eid });
  }
  async getPeer(eid) {
...
// src/oapp/factory.ts
var createOAppFactory = (contractFactory) => pMemoize(async (point) => new OApp(await contractFactory(point)));
...
var createOAppReadFactory = (contractFactory) => pMemoize(async (point) => new OAppRead(await contractFactory(point)));
var createOwnableFactory = (sdkFactory) => pMemoize(async (point) => Object.assign(await sdkFactory(point), OwnableMixin));
var EVMViewFunctionBase = class {
  constructor(eid, provider, logger11 = createModuleLogger(`EVM View SDK ${new.target.name} @ ${formatEid(eid)}`)) {
    this.eid = eid;
...
var ComputerEVM = class extends EVMViewFunctionBase {
  async resolve(cmd, compute, timeMarker, responses) {
    ...
    try {
      const mapper = computeSetting === ComputeSetting.OnlyReduce ? (this.logger.info("OnlyReduce setting is used. Skipping map step."), (r) => r.response) : (r) => this.lzMap(compute, r, timeMarker);
      const reducer = computeSetting === ComputeSetting.OnlyMap ? (this.logger.info("OnlyMap setting is used. Skipping reduce step."), (mappedResponses) => mappedResponses.join("")) : (mappedResponses) => this.lzReduce(compute, cmd, mappedResponses, timeMarker);
...
var BytesSchema = z.string().startsWith("0x");
...
var EVMTimeMarkerValidatorChain = class {
  ...
  async checkResolvedTimeMarkerValidity(tms) {
    const blockNumberToFetch = dedup((a, b) => a === b)(
...
export { BytesSchema, ComputerEVM, ContractNotFoundError, ERC20, EVMTimeMarkerResolverChain, EVMTimeMarkerValidatorChain, EVMViewFunctionBase, LzApp, OApp, OAppRead, Ownable, OwnableMixin, SingleViewFunctionCallerEVM, createERC20Factory, createLzAppFactory, createOAppFactory, createOAppReadFactory, createOwnableFactory };

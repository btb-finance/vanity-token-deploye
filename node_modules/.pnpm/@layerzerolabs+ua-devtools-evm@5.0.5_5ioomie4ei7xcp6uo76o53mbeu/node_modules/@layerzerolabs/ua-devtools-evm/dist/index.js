...
var require_bn = __commonJS({
  "../../node_modules/.pnpm/bn.js@5.2.1/node_modules/bn.js/lib/bn.js"(exports, module) {
    (function(module2, exports2) {
      ...
      BN2.prototype.isZero = function isZero3() {
...
var OwnableMixin = {
  async getOwner() {
    ...
    const owner = await devtools.mapError(
      async () => this.contract.contract.owner(),
      (error) => new Error(`Failed to get owner for OApp ${this.label}: ${error}`)
    );
    return this.logger.debug(`Got owner: ${owner}`), devtools.ignoreZero(owner);
  },
  async hasOwner(address) {
    this.logger.debug(`Checking whether owner is ${address}`);
    return devtools.areBytes32Equal(await OwnableMixin.getOwner.apply(this), address);
  },
...
// src/ownable/sdk.ts
var Ownable = class extends devtoolsEvm.OmniSDK {
  async getOwner() {
...
], Ownable.prototype, "getOwner", 1);
__decorateClass([
  devtools.AsyncRetriable()
], Ownable.prototype, "hasOwner", 1);
var ERC20 = class extends Ownable {
  async getDecimals() {
    this.logger.verbose(`Getting token decimals`);
    const decimals = await devtools.tapError(
      () => this.contract.contract.decimals(),
      (error) => (this.logger.error(`Failed to get token decimals: ${error}`), void 0)
    );
    return devtoolsEvm.BigNumberishNumberSchema.parse(decimals);
  }
  async getName() {
    this.logger.verbose(`Getting token name`);
    const name = await devtools.tapError(
      () => this.contract.contract.name(),
      (error) => (this.logger.error(`Failed to get token name: ${error}`), void 0)
    );
    return NameSchema.parse(name);
  }
  async getSymbol() {
    this.logger.verbose(`Getting token symbol`);
    const symbol = await devtools.tapError(
      () => this.contract.contract.symbol(),
      (error) => (this.logger.error(`Failed to get token symbol: ${error}`), void 0)
    );
  ...
  async getBalanceOf(user) {
    ...
    return devtoolsEvm.BigNumberishBigIntSchema.parse(allowance);
  }
...
__decorateClass([
  devtools.AsyncRetriable()
], ERC20.prototype, "getDecimals", 1);
__decorateClass([
  devtools.AsyncRetriable()
], ERC20.prototype, "getName", 1);
__decorateClass([
  devtools.AsyncRetriable()
], ERC20.prototype, "getSymbol", 1);
__decorateClass([
  devtools.AsyncRetriable()
], ERC20.prototype, "getBalanceOf", 1);
__decorateClass([
  devtools.AsyncRetriable()
], ERC20.prototype, "getAllowance", 1);
var NameSchema = zod.z.string();
var SymbolSchema = zod.z.string();
var createERC20Factory = (contractFactory) => pMemoize__default.default(async (point) => new ERC20(await contractFactory(point)));
var LzApp = class extends devtoolsEvm.OmniSDK {
  constructor(contract) {
    super(contract);
  }
  async getTrustedRemote(eid) {
    this.logger.debug(`Getting trusted remote for eid ${eid} (${devtools.formatEid(eid)})`);
    try {
      return devtools.ignoreZero(await this.contract.contract.getTrustedRemoteAddress(eid));
    } catch (error) {
      const parsedError = devtoolsEvm.parseGenericError(error);
      if ((parsedError == null ? void 0 : parsedError.reason) === "LzApp: no trusted path record") {
        return void 0;
      }
      this.logger.debug(`Got an error getting trusted remote for eid ${eid} (${devtools.formatEid(eid)}): ${error}`);
      throw error;
    }
  }
  async hasTrustedRemote(eid, address) {
    this.logger.debug(`Checking trusted remote for eid ${eid} (${devtools.formatEid(eid)})`);
    return devtools.areBytes32Equal(address, await this.getTrustedRemote(eid));
  }
  async setTrustedRemote(eid, address) {
    const description = `Setting trusted remote for eid ${eid} (${devtools.formatEid(eid)}) to address ${devtools.makeBytes32(address)}`;
    this.logger.debug(description);
    const data = this.contract.contract.interface.encodeFunctionData("setTrustedRemoteAddress", [
      eid,
      devtools.makeBytes32(address)
    ]);
    return {
      ...this.createTransaction(data),
      description
...
__decorateClass([
  devtools.AsyncRetriable()
], LzApp.prototype, "getTrustedRemote", 1);

// src/lzapp/factory.ts
var createLzAppFactory = (contractFactory) => pMemoize__default.default(async (point) => new LzApp(await contractFactory(point)));
var OApp = class extends Ownable {
  async getEndpointSDK() {
    this.logger.debug(`Getting EndpointV2 SDK`);
    const address = await devtools.mapError(
    ...
    if (devtools.isZero(address)) {
      throw new Error(
        `EndpointV2 cannot be instantiated: EndpointV2 address has been set to a zero value for OApp ${devtoolsEvm.formatOmniContract(
          this.contract
        )}`
      );
    }
    this.logger.debug(`Got EndpointV2 address: ${address}`);
    return new protocolDevtoolsEvm.EndpointV2(this.contract.contract.provider, { address, eid: this.point.eid });
  }
  async getPeer(eid) {
    const eidLabel = `eid ${eid} (${devtools.formatEid(eid)})`;
    this.logger.debug(`Getting peer for ${eidLabel}`);
    const peer = await devtools.mapError(
      () => this.contract.contract.peers(eid),
      (error) => new Error(`Failed to get peer for ${eidLabel} for OApp ${this.label}: ${error}`)
    );
    return devtools.denormalizePeer(devtools.normalizePeer(peer, this.contract.eid), eid);
  }
  async hasPeer(eid, address) {
    const peer = await this.getPeer(eid);
    return devtools.areBytes32Equal(devtools.normalizePeer(peer, eid), devtools.normalizePeer(address, eid));
  }
  async setPeer(eid, address) {
    const eidLabel = devtools.formatEid(eid);
    const normalizedPeer = await devtools.mapError(
      async () => devtools.normalizePeer(address, eid),
      (error) => new Error(`Failed to convert peer ${address} for ${eidLabel} for OApp ${this.label} to bytes: ${error}`)
    );
    const peerAsBytes32 = devtools.makeBytes32(normalizedPeer);
  ...
  async getDelegate() {
    ...
    const delegate = await devtools.mapError(
  ...
  async isDelegate(delegate) {
    ...
    const endpointSdk = await this.getEndpointSDK();
    const isDelegate = await devtools.mapError(
  ...
  async getEnforcedOptions(eid, msgType) {
    ...
    return await devtools.mapError(
  ...
  async setEnforcedOptions(enforcedOptions) {
    this.logger.debug(`Setting enforced options to ${ioDevtools.printJson(enforcedOptions)}`);
    ...
    return {
      ...this.createTransaction(data),
      description: `Setting enforced options to ${ioDevtools.printJson(enforcedOptions)}`
    };
  }
  ...
  async getCallerBpsCap() {
    if (this.contract.contract.interface.functions["callerBpsCap()"] == null) {
      return this.logger.warn(`Cannot get callerBpsCap for ${this.label}: callerBpsCap function is not supported`), void 0;
    }
    const callerBpsCap = await this.contract.contract.callerBpsCap();
    return devtoolsEvm.BigNumberishBigIntSchema.parse(callerBpsCap);
  }
  /**
   ...
   */
  serializeExecutorOptions(oappEnforcedOptionParam) {
    return oappEnforcedOptionParam.map(({ eid, option: { msgType, options } }) => ({ eid, msgType, options }));
  }
};
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "getEndpointSDK", 1);
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "getPeer", 1);
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "getDelegate", 1);
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "isDelegate", 1);
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "getEnforcedOptions", 1);
__decorateClass([
  devtools.AsyncRetriable()
], OApp.prototype, "getCallerBpsCap", 1);

// src/oapp/factory.ts
var createOAppFactory = (contractFactory) => pMemoize__default.default(async (point) => new OApp(await contractFactory(point)));
var OAppRead = class extends OApp {
  async setReadChannel(channelId, active) {
  ...
  async isReadChannelActive(channelId) {
    ...
    return devtools.areBytes32Equal(
      devtools.normalizePeer(peer, this.contract.eid),
      devtools.normalizePeer(this.contract.contract.address, this.contract.eid)
    );
  }
};

// src/oapp-read/factory.ts
var createOAppReadFactory = (contractFactory) => pMemoize__default.default(async (point) => new OAppRead(await contractFactory(point)));
var createOwnableFactory = (sdkFactory) => pMemoize__default.default(async (point) => Object.assign(await sdkFactory(point), OwnableMixin));
var EVMViewFunctionBase = class {
  constructor(eid, provider, logger11 = ioDevtools.createModuleLogger(`EVM View SDK ${new.target.name} @ ${devtools.formatEid(eid)}`)) {
    this.eid = eid;
    this.provider = provider;
    this.logger = logger11;
  }
  async callContract(callData, toAddress, blockNumber) {
    this.logger.debug(
      `Calling contract on chain ${devtools.formatEid(this.eid)}, address ${"0x" + toAddress}, calldata ${callData} at blockNumber ${blockNumber}`
    );
    const result = await devtools.mapError(
      () => this.provider.call(
        {
          to: "0x" + toAddress,
          data: callData
      ...
      (error) => {
        const parsedError = devtoolsEvm.parseGenericError(error);
        this.logger.error(`Error calling contract on chain ${devtools.formatEid(this.eid)} : ${parsedError}`);
        return parsedError;
      }
    );
    await devtools.tapError(
      () => BytesSchema.parse(result),
      (error) => {
        this.logger.error(`Error parsing result from contract call on chain ${devtools.formatEid(this.eid)} : ${error}`);
      }
    );
    if (!devtools.isZero(result)) {
      return result.replace("0x", "");
    }
    this.logger.debug(
      `0x returned, checking if target contract is deployed on chain ${devtools.formatEid(this.eid)}, address ${"0x" + toAddress} at blockNumber ${blockNumber}`
    );
    const byteCode = await this.provider.getCode(toAddress, blockNumber);
    if (devtools.isZero(byteCode)) {
...
var NumberCoder = class extends Coder {
  ...
  encode(writer, value) {
    ...
    let maxUintValue = constants.MaxUint256.mask(writer.wordSize * 8);
    if (this.signed) {
      let bounds = maxUintValue.mask(this.size * 8 - 1);
      if (v.gt(bounds) || v.lt(bounds.add(constants.One).mul(constants.NegativeOne))) {
        this._throwError("value out-of-bounds", value);
      }
    } else if (v.lt(constants.Zero) || v.gt(maxUintValue.mask(this.size * 8))) {
      this._throwError("value out-of-bounds", value);
    }
...
var iOAppMapperAbi = ["function lzMap(bytes calldata _request, bytes calldata _response) view returns (bytes)"];
var iOAppReducerAbi = ["function lzReduce(bytes calldata _cmd, bytes[] calldata _responses) view returns (bytes)"];
var ComputerEVM = class extends EVMViewFunctionBase {
  async resolve(cmd, compute, timeMarker, responses) {
    const computeSetting = compute.computeHeader.computeSetting;
    this.validateComputeSetting(computeSetting);
    try {
      const mapper = computeSetting === lzV2Utilities.ComputeSetting.OnlyReduce ? (this.logger.info("OnlyReduce setting is used. Skipping map step."), (r) => r.response) : (r) => this.lzMap(compute, r, timeMarker);
      const reducer = computeSetting === lzV2Utilities.ComputeSetting.OnlyMap ? (this.logger.info("OnlyMap setting is used. Skipping reduce step."), (mappedResponses) => mappedResponses.join("")) : (mappedResponses) => this.lzReduce(compute, cmd, mappedResponses, timeMarker);
      const applicative = devtools.createDefaultApplicative(this.logger);
      const mapped = await applicative(responses.map((r) => () => mapper(r)));
      const reduced = await reducer(mapped);
      return reduced;
    } catch (error) {
      if (error instanceof ContractNotFoundError || error instanceof devtoolsEvm.RevertError) {
        this.logger.error(`Error on resolving compute section of command: ${error}`);
        throw new uaDevtools.UnresolvableCommandError();
      }
      throw error;
    }
  }
  validateComputeSetting(computeSetting) {
    assert__default.default(
      [lzV2Utilities.ComputeSetting.OnlyMap, lzV2Utilities.ComputeSetting.OnlyReduce, lzV2Utilities.ComputeSetting.MapReduce].includes(computeSetting),
      `Unsupported compute setting: ${computeSetting}`
    );
  }
  async lzMap(compute, requestResponsePair, timeMarker) {
...
var SingleViewFunctionCallerEVM = class extends EVMViewFunctionBase {
  async resolve(request, timeMarker) {
    try {
...
// src/read/errors.ts
var ContractNotFoundError = class extends Error {
  constructor() {
    super("Contract not found at address");
  }
};
var BytesSchema = zod.z.string().startsWith("0x");
var updateLowerBoundBlock = (lowerBoundBlock, potentialBlock) => {
  return !lowerBoundBlock || lowerBoundBlock.number < potentialBlock.number ? potentialBlock : lowerBoundBlock;
};
var updateUpperBoundBlock = (upperBoundBlock, potentialBlock) => {
  return !upperBoundBlock || upperBoundBlock.number > potentialBlock.number ? potentialBlock : upperBoundBlock;
};
var calculateAvgBlockTime = (leftBlock, rightBlock) => {
  assert__default.default(rightBlock.number != leftBlock.number, "Invalid block: The two blocks have the same number");
...
var EVMTimeMarkerResolverChain = class {
  constructor(eid, provider) {
    this.eid = eid;
    this.provider = provider;
  }
  async resolveTimestamps(timestamps) {
    const provider = this.provider;
    let avgBlockTime = 1e3;
    ...
    await Promise.all(
      uniqueTimestamps.map(async (targetTimestamp) => {
        ...
        let upperBoundBlock = latestBlock;
        let lowerBoundBlock = null;
        let iterate = true;
        while (iterate) {
          ...
          if (uaDevtools.isBlockMatchingTimestamp(nextBlock, nextBlockMinusOne, targetTimestamp)) {
          ...
          const updatedAvgBlockTime = lowerBoundBlock ? calculateAvgBlockTime(lowerBoundBlock, upperBoundBlock) : calculateAvgBlockTime(nextBlock, latestBlock);
...
var EVMTimeMarkerValidatorChain = class {
  constructor(eid, provider) {
    this.eid = eid;
    this.provider = provider;
  }
  async checkResolvedTimeMarkerValidity(tms) {
    const blockNumberToFetch = uaDevtools.dedup((a, b) => a === b)(
      tms.flatMap((tms2) => [tms2.blockNumber, Math.max(tms2.blockNumber - 1, 1)])
    );
    ...
    for (const tm of tms) {
      ...
      const previousBlockTime = blockNumber === 1 ? null : {
        number: blockNumber - 1,
        timestamp: timestampMapping[blockNumber - 1]
      };
      if (!uaDevtools.isBlockMatchingTimestamp(blockTime, previousBlockTime, timestamp)) {
        throw new Error(
          `Invalid resolved time marker for eid ${this.eid}: blockNumber ${blockNumber} with resolved timestamp ${timestamp} does not meet actual timestamp for blockNumber ${blockTime.timestamp} and previous blockNumber ${previousBlockTime == null ? void 0 : previousBlockTime.timestamp}`
        );
      }
    }
  }
...
exports.ComputerEVM = ComputerEVM;
exports.ContractNotFoundError = ContractNotFoundError;
exports.ERC20 = ERC20;
exports.EVMTimeMarkerResolverChain = EVMTimeMarkerResolverChain;
exports.EVMTimeMarkerValidatorChain = EVMTimeMarkerValidatorChain;
exports.EVMViewFunctionBase = EVMViewFunctionBase;
exports.LzApp = LzApp;
exports.OApp = OApp;
exports.OAppRead = OAppRead;
exports.Ownable = Ownable;
exports.OwnableMixin = OwnableMixin;
exports.SingleViewFunctionCallerEVM = SingleViewFunctionCallerEVM;
exports.createERC20Factory = createERC20Factory;
exports.createLzAppFactory = createLzAppFactory;
exports.createOAppFactory = createOAppFactory;
exports.createOAppReadFactory = createOAppReadFactory;
exports.createOwnableFactory = createOwnableFactory;

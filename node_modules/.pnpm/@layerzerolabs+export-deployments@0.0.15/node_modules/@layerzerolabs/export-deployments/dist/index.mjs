import { lstatSync, mkdirSync, writeFileSync, opendirSync } from 'fs';
import { basename, dirname, resolve } from 'path';
import { factory, SyntaxKind, isExpressionStatement, NodeFlags, createPrinter, createSourceFile, ScriptTarget, ScriptKind, ListFormat } from 'typescript';
...
var isDirectorySafe = tryCatchK((path) => lstatSync(path).isDirectory(), toError);
var mkdirSafe = tryCatchK((path) => (mkdirSync(path, { recursive: true }), path), toError);
var writeFileSyncSafe = tryCatchK(({ content, path }) => writeFileSync(path, content), toError);
var listEntriesSafe = (predicate) => tryCatchK((path) => {
...
var createConst = (modifiers = []) => (name, type) => (expression) => factory.createVariableStatement(
  modifiers,
  factory.createVariableDeclarationList(
    [factory.createVariableDeclaration(name, void 0, type, expression)],
    NodeFlags.Const
  )
);
var createTypeOf = factory.createTypeQueryNode;
var createTypeReferenceNode = factory.createTypeReferenceNode;
var createTypeDeclaration = (modifiers = []) => (name) => (type) => factory.createTypeAliasDeclaration(modifiers, name, void 0, type);
var creteAsConst = (expression) => factory.createAsExpression(expression, createTypeReferenceNode(createIdentifier("const"), void 0));
var createIdentifier = factory.createIdentifier;
var createStringLiteral = factory.createStringLiteral;
var createArrayLiteral = (expressions) => factory.createArrayLiteralExpression(expressions, false);
var createExportConst = createConst([factory.createToken(SyntaxKind.ExportKeyword)]);
var normalizeIdentifierName = (name) => name.replaceAll("-", "_");
var createExportAsterisk = (name, path = `./${name}`) => factory.createExportDeclaration(
  void 0,
  false,
...
var recordToObjectLiteral = (0, import_function7.flow)(
  collect(import_string.Ord)(
    (key, value) => factory.createPropertyAssignment(factory.createStringLiteral(key), value)
  ),
  (properties) => factory.createObjectLiteralExpression(properties, true)
);
var recordToRecordType = (0, import_function7.flow)(
  collect(import_string.Ord)(
    (key, value) => factory.createPropertySignature(
      void 0,
...
var createJSONSourceFileSafe = tryCatchK(
  (content) => createSourceFile("file.json", content, ScriptTarget.JSON, false, ScriptKind.JSON),
  toError
);
var runtimeObjectToExpressionSafe = (0, import_function7.flow)(
  ...
  filterOrElse2(isExpressionStatement, () => new Error("Malformed JSON file")),
...
export { createIncludeDirent, generate3 as generate, generateSafe, generate2 as generatorMarkdown, generate as generatorTypeScript };
